...
from .forms import PhoneForm, PersonSearchForm, EmailSearchForm, PhoneSearchForm
...
class PhoneSearchView(LoginRequiredMixin, FormMixin, ListView):
    """ Search phone view accessible after login """

    template_name = 'contact_book/phone_search.html'
    model = Phone
    ...
    def get_queryset(self):
        ...
        if form.is_valid():
            return Phone.objects.filter(
                person__first_name__icontains=form.cleaned_data['first_name'],
                person__second_name__icontains=form.cleaned_data['second_name'],
                phone__icontains=form.cleaned_data['phone'],
                person__owner=user).order_by('-person__date_added')

        return Phone.objects.filter(person_owner=user).order_by('-date_added')
